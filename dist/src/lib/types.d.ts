...
export type ChatItemType = StreamingStep | ({
    role: "user";
} & ContentString);
...
export type ChatGPTMessage = SystemMessage | AssistantMessage | UserMessage | {
    role: "function";
    content: string;
    name: string;
    summary?: string;
...
type NonSystemGPTMessage = Exclude<ChatGPTMessage, {
    role: "system";
}>;
...
export type StreamingStepInput = NonSystemGPTMessage | ErrorMessage | DebugMessage | ConfirmationMessage | LoadingMessage | GraphMessage;
export type StreamingStep = StreamingStepInput & {
    id: number;
};
